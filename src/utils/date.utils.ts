...
export function formatDate(date: Date | null, format: DateFormat = DateFormat.YYYYMMDD): string {
    if (!date) {
        return '';
    }
    const day = date.getDate();
    ...
    let formattedDate = format
        ...
        .replace('HH:mm:ss', `${get24HourFormat(date)}:${getMinutes(date)}:${getSeconds(date)}`);
