...
export function ThemedOTPInput(
  ...
  useEffect(() => {
    const checkClipboard = async () => {
      const content = await Clipboard.default.getString();
      if (content && content.length === props.otpLength) {
    ...
    const intervalId = setInterval(checkClipboard, 1000); // Check every second
